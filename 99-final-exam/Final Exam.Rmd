...
```{r}
femur_length <- 25:50
prob_male <- (dnorm(femor_length,40,3.4)) /((dnorm(femor_length,40,3.4)) + (dnorm(femor_length,36,3.3)))
plot.new()
...
```{r}
set.seed(1)
meds_out <- rep(NA,1000)
for (i in seq(1:1000)){
  meds<- (rlnorm(101,0,1))
  meds_out[i]<-quantile(meds,0.5)
}
hist(meds_out, breaks = 50)
plot(density(meds_out))
```
...
hist(female_adult_bmi,breaks=20,freq = F,xlim = c(0, 100))
...
#num replicates
mean_sample_smoke <- rep(NA, 1000)
for(i in 1:1000){
  sample_smoke <- rnorm(length(bwt_s), mean = mean_mle_smoke, sd = sd_mle_smoke)
  mean_sample_smoke[i] <- mean(sample_smoke)
}
conf <- quantile(mean_sample_smoke, c(0.025, 0.975))
conf
```
