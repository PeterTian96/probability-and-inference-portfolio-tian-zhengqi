...
<p>&emsp; To answer the question, we need to consider the definition of the win opportunity. Braves needs to win at least 4 times in 7 games, or most 3 loses in 7 games. As we know the Pb and Py, we will able to have the following expressions </p>
```{r}
Pb<- 0.55
Py<- 1-Pb
Brave_Win_Prob<- 
...
<p>&emsp; As we know that the Pb needs to be 0.55, we want to observe the outcome as the game series length change. In fact, we want the total length be a odd number, so that one team is able to win. we assume the game around could up to 9999999. When the probability is equal to or more than 0.8, we find it as the shortest series length and probability for the shortest series.</p>
